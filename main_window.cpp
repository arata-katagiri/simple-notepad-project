...
void main_window::setup_tools_menu()
{
    ...
    const auto* action_check_spell = tools_menu->addAction("Check Spelling...");
    connect(action_check_spell, &QAction::triggered, this, [this] {
        highlighter->rehighlight();
    });
    connect(editor, &QWidget::customContextMenuRequested, this, [this](const QPoint& pos) {
        ...
        if (!checker.is_correct(cursor.selectedText().toStdString())) {
            ...
            if (auto* action = menu->exec(editor->mapToGlobal(pos))) {
